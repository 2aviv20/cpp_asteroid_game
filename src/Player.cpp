...
{
	this->playerBody = new Body(PolygonShape({ Vec2(-20, 0), Vec2(0, 20),Vec2(20, 0) }), windowWidth / 2,windowHeight / 2, 1.0,1.0f);
}
...
void Player::Draw()
{
	...
	Graphics::DrawPolygon(this->playerBody->position.x, this->playerBody->position.y, polygon->worldVertices, 0xFFFFFFFF,this->playerBody->scale);
}
